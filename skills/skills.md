...
## KATANAOS Klipper Flash-Logik (Critical)
**Regel:** Die Flash-Methode wird dynamisch anhand des Build-Artefakts bestimmt, nicht statisch ausgewählt.

1.  **Erkennungs-Logik (The Selector):**
    * **IF** `out/klipper.uf2` existiert (RP2040, SAMD):
        * **Methode:** Mass Storage Copy (Mount & Copy).
        * **Verbot:** Niemals `dfu-util` oder `make flash` aufrufen.
        * **Ablauf:** Suche Block-Device (Label: `RPI-RP2` o.ä.) -> Mounten -> `cp out/klipper.uf2 /mnt/...` -> `sync`.
    * **IF** `out/klipper.bin` existiert (STM32, AVR):
        * **Methode:** DFU oder Serial.
        * **Ablauf:** Prüfe mit `lsusb` auf DFU-ID (z.B. `0483:df11`) -> Nutze `dfu-util`. Falls nicht -> Nutze Serial Flash (`make flash`).

2.  **Fehlervermeidung:**
    * Zeige dem User bei Vorhandensein einer `.uf2` Datei niemals die Option "DFU-Flash" an. Das ist technisch unmöglich und führt zum Abbruch.
